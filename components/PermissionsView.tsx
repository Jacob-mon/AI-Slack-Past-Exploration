...
const PermissionItem: React.FC<PermissionItemProps> = ({ scope, description, hasPermission }) => (
    <li className="flex items-start space-x-4 p-3 bg-gray-700/50 rounded-lg">
        <div>
            {hasPermission ? (
                ...
                <ul className="space-y-3">
                    {requiredScopes.map(scope => (
                        <PermissionItem 
                            key={scope}
                            scope={scope}
                            description={scopeDescriptions[scope] || '필수 권한입니다.'}
                            hasPermission={userScopes.has(scope)}
                        />
                    ))}
                </ul>
            </div>
